...
const sassPlugin = (options) => ({
  ...
  async setup(build) {
    ...
    build.onLoad({ filter: /\.(sass|scss)$/ }, async (args) => {
      if (!sass) {
        console.error(
          "error: Sass is not installed. Try running `npm i sass -D` and then building again.",
        );
        return;
      }
    ...
    build.onEnd(async (result) => {
      ...
      const stripPrefix = (str) =>
        str
          .replace(/^frontend\//, "")
          .replace(
            RegExp(String.raw`^${bridgetownConfig.source}\/${bridgetownConfig.islandsDir}\/`),
            "islands/",
          );
...
export default async (esbuildOptions, ...args) => {
  ...
  const islands = glob
    .sync(`./${bridgetownConfig.source}/${bridgetownConfig.islandsDir}/*.{js,js.rb}`)
    .map((item) => `./${item}`);
