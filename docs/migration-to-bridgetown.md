...
`src/consts.ts` の内容を YAML として `src/_data/site_metadata.yml` に転記する。あわせて `config/initializers.rb` にサイト固有の設定（`url`, `timezone`, 日本語 locale）を反映する。

```ruby
# config/initializers.rb（雛形に追記する箇所）
...
`available_locales` に `:ja` を含めないと、日本語ページのレンダリング時に `I18n::InvalidLocale` で落ちるので忘れずに設定する。
...
> **実装上の注意**: Step 1 完了直後に、Bridgetown が Astro の `.astro` ファイルを Resource として読みに行ってビルドが落ちることが判明した。`exclude` 設定（initializer DSL でも `bridgetown.config.yml` でも）が `src/` 配下のスキャンを抑止できなかったため、本ステップ（投稿の移行）と Step 10（Astro 撤去）を Step 2 の直後に前倒しで実施することにした。並行運用は断念。
...
> **実装上の注意**: Step 6 と同じ理由で、本ステップは Step 2 の直後（Step 6 の直後）に前倒しで実施した。並行運用を断念したため、ローカル動作確認（Step 9）を待たずに Astro を撤去する流れになる。
